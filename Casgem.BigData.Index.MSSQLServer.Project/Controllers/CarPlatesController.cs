...
namespace Casgem.BigData.Index.MSSQLServer.Project.Controllers
{
    public class CarPlatesController : Controller
    {
        ...
        public IActionResult Index_2()
        {
            ...
            var query1 = "SELECT * FROM MAX_BRAND_AND_MODEL();";
            ...
            var query2 = "SELECT * FROM MIN_CAR_COLOR();";
            var result2 = (List<MinimumColorDto>)CONNECTION.Query<MinimumColorDto>(query2); //liste olmayacak!
            Console.Write(result2[0].RENK);
            Console.Write(result2[0].RENK_ADETI);
            ...
            var query3 = "SELECT * FROM MAX_LICENCE_DATE();";
            ...
            var query4 = "SELECT * FROM ISTANBUL_MIN_CAR();";
            ...
            var query5 = "SELECT * FROM HIGH_MOTOR_VOLUME();";
            ...
            var query6 = "SELECT * FROM ISTANBUL_MANUEL_SHIFT_TYPE();";
            ...
            var query7 = "SELECT * FROM ISTANBUL_AUTOMATIC_SHIFT_TYPE();";
        ...
        {
            string query = "SELECT TOP 100 BRAND, PLATE, SHIFTTYPE, FUEL FROM PLATES";
            var searchResults = CONNECTION.Query<SearchQueryResultViewModel>(query);
            return View(searchResults);
        }

        [HttpPost]
        public IActionResult Index_3(string searchQuery)
        {
            string query = @"
            SELECT TOP 100 BRAND, PLATE, SHIFTTYPE, FUEL
            FROM PLATES
            WHERE BRAND LIKE '%' + @Search + '%'
               OR PLATE LIKE '%' + @Search + '%'
               OR SHIFTTYPE LIKE '%' + @Search + '%'
               OR FUEL LIKE '%' + @Search + '%'";
            var searchResults = CONNECTION.Query<SearchQueryResultViewModel>(query, new { Search = searchQuery });
            return View(searchResults);
        }
